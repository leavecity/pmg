...
Files marked `Status: deprecated` or `Status: archived` are excluded from default context bundles. Files under `.pmg/memory/proposals/` and `.pmg/memory/archive/` are also excluded. PMG keeps pending proposals, inactive memory, and audit records available for review, cleanup workflows, and historical analysis, but they should not guide normal agent work.
