...
- `pmg context explain`: explain selected, candidate, and excluded context sources without emitting the full context bundle; supports source filters such as `--no-reviews` and `--no-specs`
